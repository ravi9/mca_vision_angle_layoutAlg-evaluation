function [Lines Layout Images t_mag] = MagnetVision (Lines, AffinityEnergyMatrix, MinGap, dimRedAlg, MagnetDiameter, MagnetThickness, Units, SizeX, SizeY, SaveFlag, ImageFile)
...
%%
%% MagnetDiameter: -- Diameter of the magnet (nm)
%%
...
[Y, mapping] = compute_mapping(Distances, dimRedAlg, 2);
...
if SaveFlag
    ...
    % bitmap
    MagnetShape = CreateMagnetShape (MagnetDiameter);
    MaskImage = conv2(ExpandedField, MagnetShape, 'same');
    %ImageFile = 'testing';
    fprintf(1, '\n Writing coordinates of magnets in text file Coords.txt');
    fp = fopen (sprintf('Results_dimRedAlg_%s/Coords_%s_%d_g_%d_d_%d_u_%d.txt',dimRedAlg, ImageFile, i, MinGap*Units, MagnetDiameter*Units, Units), 'w');
    ...
    imwrite(mat2gray(MaskImage), sprintf('Results_dimRedAlg_%s/VisionDots_%s_%d_g_%d_d_%d_u_%d.bmp',dimRedAlg, ImageFile, i, MinGap*Units, MagnetDiameter*Units, Units), 'bmp');
...
[Angles States TimeTaken]  = NanodiskArray (X, Y, MagnetDiameter/2, MagnetThickness, Input, pi*ones(size(X))/4, [0 h]);
