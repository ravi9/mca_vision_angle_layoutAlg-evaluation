...
for i=1:length(dimensionRedAlgList)
    ...
    resultDir = ['Results_dimRedAlg_' dimRedAlg ImageFileNameSplit{2}];
    
    if exist([resultDir '_AllOutput.mat']) == 0
        try
            [Lines Layout MOutput t_magnet] = MagnetVision (Lines, AffinityEnergyMatrix, MinGap,dimRedAlg, MagnetDiameter, MagnetThickness, Units, SizeX, SizeY, SaveFlag, ImageFile);
        ...
        catch magnetVisionException
            fprintf('\n');
            disp([' #*#*# ERROR: Failed Execution. Alg: ' dimRedAlg ', File: ' ImageFile ]);
